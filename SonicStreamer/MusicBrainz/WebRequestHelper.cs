...
namespace SonicStreamer.MusicBrainz
{
    internal static class WebRequestHelper
    {
        ...
        /// </summary>
        internal static string CreateBrowseTemplate(string entity, string relatedEntity, string mbid, int limit,
            int offset, string inc)
        {
            return string.Format("{0}{1}", WebServiceUrl,
                string.Format(BrowseTemplate, entity, relatedEntity, mbid, limit, offset, inc));
        }
        ...
        internal static T DeserializeStream<T>(Stream stream, bool withoutMetadata) where T : Entities.Entity
        {
            if (stream == null)
            {
                throw new NullReferenceException(Constants.MusicBrainzEmptyStream);
            }
            ...
            {
                throw new NullReferenceException(Constants.MusicBrainzWrongResponseFormat);
            }
