...
namespace SonicStreamer.MusicBrainz.Entities
{
    ...
    public abstract class Entity
    {
        ...
        protected async static Task<T> GetAsync<T>(string entity, string id, params string[] inc) where T : Entity
        {
            ...
            {
                throw new ArgumentException(string.Format(Constants.MusicBrainzMissingParameter, "entity"));
            }

            if (string.IsNullOrEmpty(id))
            {
                throw new ArgumentException(string.Format(Constants.MusicBrainzMissingParameter, "id"));
            }

            return
                await WebRequestHelper.GetAsync<T>(WebRequestHelper.CreateLookupUrl(entity, id, CreateIncludeQuery(inc)));
        }
        ...
        /// <returns></returns>
        protected async static Task<T> SearchAsync<T>(string entity, string query, int limit = 25, int offset = 0)
            where T : MetadataWrapper
        {
            if (string.IsNullOrEmpty(entity))
            {
                throw new ArgumentException(string.Format(Constants.MusicBrainzMissingParameter, "entity"));
            ...
            {
                throw new ArgumentException(string.Format(Constants.MusicBrainzMissingParameter, "query"));
            }
        ...
        /// <param name="inc">A list of entities to include (subqueries).</param>
        /// <returns></returns>
        protected async static Task<T> BrowseAsync<T>(string entity, string relatedEntity, string relatedEntityId,
            int limit, int offset, params string[] inc) where T : Entity
        {
            if (string.IsNullOrEmpty(entity))
            {
                throw new ArgumentException(string.Format(Constants.MusicBrainzMissingParameter, "entity"));
